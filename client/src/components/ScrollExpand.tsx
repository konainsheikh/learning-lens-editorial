...
export default function ScrollExpand({
  ...
  enabled = true, className = "", onProgress, onRelease, onRetract,
}: ScrollExpandProps) {
  ...
  const applyProgress = useCallback((progress: number) => {
    ...
    const overlayEnter = smoothstep(.68, 1, progress);
  ...
  return <div ref={rootRef} className={`scroll-expand scroll-expand--window ${className}`.trim()}>
    <div ref={trackRef} className="scroll-expand__track">
      <div ref={stageRef} className="scroll-expand__stage">
        <div ref={frameRef} className="scroll-expand__frame"><img ref={mediaRef} className="scroll-expand__media" src={src} alt={alt} draggable={false} /><div ref={scrimRef} className="scroll-expand__scrim" />{children ? <div ref={overlayRef} className="scroll-expand__overlay">{children}</div> : null}</div>
